// CODIGO INSERIDO NO ARDUINO DA PLANTA
// LINK PARA SIMULAÇÃO DE TESTE:
// https://www.tinkercad.com/things/5CSvp5RoUG0-mistura-de-liquidos-original/editel?sharecode=br0Lwb0LfuK4qN2Wq12zQGO0n7RWnbu23T3J_V8_aNk

#include <Arduino.h>
#include <LiquidCrystal.h>

LiquidCrystal led(12, 13, A2, A3, A4, A5);

// Definições dos pinos
#define PINO_BOMBA_1 8
#define PINO_BOMBA_2 9

#define PINO_BOTAO1 A0
#define PINO_BOTAO2 A1

#define PINO_MISTURADOR 7
#define PINO_SOLENOIDE 6

#define PINO_SENSOR_VAZIO 5
#define PINO_SENSOR_CHEIO 4
...
const uint8_t btns[numBtn] = {A0, A1};
...
void acionar_bomba(int in1, bool acao)
{
    if (acao == LIGAR_BOMBA)
    {
...
void setup()
{
    ...
    acionar_bomba(PINO_BOMBA_1, DESLIGAR_BOMBA);
    acionar_bomba(PINO_BOMBA_2, DESLIGAR_BOMBA);
...
void loop()
{
    ...
    case ESTADO_BOMBA1:
        ...
        acionar_bomba(PINO_BOMBA_1, LIGAR_BOMBA);
        tempoDeInicio = millis();
    ...
    case ESTADO_BOMBA2:
        ...
        acionar_bomba(PINO_BOMBA_2, LIGAR_BOMBA);
...
/* put your main code here, to run repeatedly:
  novo_estado_sensor_vazio = digitalRead(PINO_SENSOR_VAZIO);
  if(novo_estado_sensor_vazio != estado_sensor_vazio)
  {
    estado_sensor_vazio = novo_estado_sensor_vazio;
    Serial.print("Sensor VAZIO variou - ");
    Serial.println(estado_sensor_vazio);
  }

  novo_estado_sensor_cheio = digitalRead(PINO_SENSOR_CHEIO);
  if(novo_estado_sensor_cheio != estado_sensor_cheio)
  {
    estado_sensor_cheio = novo_estado_sensor_cheio;
    Serial.print("Sensor CHEIO variou - ");
    Serial.println(estado_sensor_cheio);
  }
